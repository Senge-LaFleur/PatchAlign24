    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for n_iter, batch in enumerate(loop):
                bs = len(batch['mid'])
                textual_embeddings = torch.cat(tuple([torch.tensor(text_embeddings).unsqueeze(0)]*bs)).to(device).double()
...
if __name__ == '__main__':
    ...
    # Fix: Ensure the column is named 'fitzpatrick' as expected by the script
    ...
    for holdout_set in [domain]: # ["expert_select","random_holdout", "a12", "a34","a56", "dermaamin","br"]:
        ...
        for indexer, label in enumerate([level]):
            ...
            dataloaders, dataset_sizes = custom_load(
                32,
                0, # Reduced from 10 to prevent Windows Paging File RAM crashes
                "{}".format(train_path),
                "{}".format(test_path),
                label = label,
                dataset_name = dataset_name)
